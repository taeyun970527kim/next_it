...
public class NextStudent {
	// 학생들의 상태(state)를 (필드 변수)로 선언
	// 필드 변수의 값을 꼭 초기화(Init)하지 않아도 된다.
	String name;
	int age;
	int level; // 기본값 설정을 필드변수에 하여도 됨 (1, 0)
	int exp;
	
	// 기본 생성자
	// 클래스 내에 생성자를 별도로 만들지 않으면 
	// 생략된 상태로 존재한다. 
//	public NextStudent() {} 이와 같은 상태로
	
	// 만약 생성자를 따로 만들어주었다면
	// 생략된 상태로 존재하던 기본 생성자가 제거된다
	// 기본 생성자 사용을 위해 따로 만들어준다.
	public NextStudent() {
		System.out.println("기본 생성자로 객체 생성");
	}
	// 생성자는 객체가 생성될 때(= new 클래스()) 실행되는 메소드라 봐도 무방 
	
	// 생성자(Constructor) 만들기
	// 단축키 [Alt + Shift + S] 또는 마우스 우클릭 - Source 탭
