...
public class NextMain {
	...
	public static void main(String[] args) {
		...
		if(changyeonExp >= 100) {
			...
			changyeonExp -= 100;
//			changyeonExp %= 100; // 둘다 똑같음 100으로 나눈 나머지 값만 표기
		}
		...
		// 2명 이상 만들어보기
		NextStudent kim = new NextStudent("김", 28, 1, 0);
		NextStudent taeyun = new NextStudent("태윤", 28, 1, 0);
		
		// 레벨과 경험치는 1과 0으로 고정
		// 객체를 생성할때, 레벨과 경험치를 건드리지 않는다.
		// 즉 이름과 나이만 입력받는 생성자로 객체를 생성하고자 한다.
...
//		junhaBot.level += 5; // call by reference
...
//		Collections.sort(stuList);
		...
		// 버블정렬을 이용하여 정렬하기
		// 변수명이나 함수명 한번에 바꾸기 단축키 [Alt + Shift + R]
		// 레벨순으로 정렬
		// stuList [지혜][태윤][자영][준호]
