...
public class ExceptionMain {
	...
	public static void main(String[] args) {
		...
		// 현업에서는 뻑났다라고 말함
		// 예외는 굳이 나누면 예측 가능한 예외와 예측하지 못한 예외로 나눌 수 있다.
		...
		try {
			...
			System.out.println("인덱싱 성공"); // 위에 값에서 오류 발생 후 바로 catch 문으로 들어가 출력 안됨.
		} catch(ArrayIndexOutOfBoundsException except) {
			// ArrayIndexOutOfBoundsException
			// try 블록 내에서 발생할 수 있는 에러 종류에 대한 클래스
			// 해당 에러가 발생하면 catch 블록 내부 실행
		...
		try {
			...
			System.out.println("예러메시지: " + e.getMessage());
			e.printStackTrace();
		}
		...
		// 일반적으로 예외처리는 충분한 테스트 이후 개발 막바지에 적용
		
		System.out.println("\n====================================================================\n");
		
		// 콘솔형 프로그램에서 숫자를 입력하지 않으면 다시 입력할 수 있도록
...
//		}
		...
		ExceptionMethod.indexing(intArray);
		
		// 에러 처리를 해주어야하기 때문에 해당 메소드를 최종적으로 실행하는 곳에서는
		// try/catch를 해주어야한다
		try {
			ExceptionMethod.printName("찬웅2");
			...
			e.printStackTrace();
			// 에러페이지로 전환(예측 못했을 때의 보험)
		}
		
		System.out.println("이름 출력 끝");
		
		System.out.println(ExceptionMethod.dateMillSec("2023.04.20"));
		
		System.out.println("\n====================================================================\n");
		
		try {
			ExceptionMethod.printName("태윤");
