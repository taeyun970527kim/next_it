...
//			String warning = "리뷰는 12자 이상 작성하세요";
		...
		// level 3
		if(name.length() > 0 
				&& (phone.length() == 10 || phone.length() == 11) // 괄호 쳐주어야함 == 10이랑 name.length가 묶이기 때문 
				&& age >= 14) {
			System.out.println("회원가입 성공");
		} else {
			System.out.println("회원가입 실패");
